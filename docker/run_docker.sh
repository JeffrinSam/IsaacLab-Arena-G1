...
# Remove any existing containers with the same name (stopped, exited, or running)
if [ "$(docker ps -a --quiet --filter name=^${DOCKER_IMAGE_NAME}-${DOCKER_VERSION_TAG}$)" ]; then
    echo "Removing existing container: ${DOCKER_IMAGE_NAME}-${DOCKER_VERSION_TAG}"
    docker stop ${DOCKER_IMAGE_NAME}-${DOCKER_VERSION_TAG} > /dev/null 2>&1 || true
    docker rm ${DOCKER_IMAGE_NAME}-${DOCKER_VERSION_TAG} > /dev/null 2>&1 || true
fi
...
# Determine dataset and model mount directories (prefer project data, fallback to home)
if [ -d "$SCRIPT_DIR/../data/datasets" ]; then
    DATASETS_MOUNT_DIR="$SCRIPT_DIR/../data/datasets"
else
    DATASETS_MOUNT_DIR="$DATASETS_HOST_MOUNT_DIRECTORY"
fi
if [ -d "$SCRIPT_DIR/../data/models" ]; then
    MODELS_MOUNT_DIR="$SCRIPT_DIR/../data/models"
else
    MODELS_MOUNT_DIR="$MODELS_HOST_MOUNT_DIRECTORY"
fi
...
if [ "$( docker container inspect -f '{{.State.Running}}' $DOCKER_IMAGE_NAME'-'$DOCKER_VERSION_TAG 2>/dev/null)" = "true" ]; then
  ...
  docker exec -it $DOCKER_IMAGE_NAME-$DOCKER_VERSION_TAG su $(id -un)
    ...
    DOCKER_RUN_ARGS=("--name" "$DOCKER_IMAGE_NAME-$DOCKER_VERSION_TAG"
                    ...
                    $(add_volume_if_it_exists "$DATASETS_MOUNT_DIR" /datasets)
                    $(add_volume_if_it_exists "$MODELS_MOUNT_DIR" /models)
    ...
    # Add -it flags for interactive terminal when no command is provided
    if [ $# -eq 0 ]; then
        docker run -it "${DOCKER_RUN_ARGS[@]}" --rm ${DOCKER_IMAGE_NAME}:${DOCKER_VERSION_TAG} "${@}"
    else
        docker run "${DOCKER_RUN_ARGS[@]}" --rm ${DOCKER_IMAGE_NAME}:${DOCKER_VERSION_TAG} "${@}"
    fi
fi
